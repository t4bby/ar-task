...
describe("User API Endpoints", () => {
    ...
    describe("GET /users", () => {
        it("should return a list of users", async () => {
            ...
            expect(responseBody.responseObject.length).toEqual(allUsers.length);
            responseBody.responseObject.forEach((user: User, index: number) => compareUsers(allUsers[index] as User, user));
        });
    });
    ...
    describe("GET /users/:id", () => {
        it("should return a user for a valid ID", async () => {
            // Arrange
            const allUsers = await userRepository.findAllAsync();
            const testId = allUsers[0]?.id ?? 1;
            const expectedUser = await userRepository.findByIdAsync(testId);
